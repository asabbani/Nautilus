...
imu_sensor_path = "/".join(split_path_sensors) + \
    "/Sensor/Adafruit_Python_BNO055/Adafruit_BNO055"
...
class AUV:
    ...
    def run(self):
        ...
        try:
            ...
            while self.is_radio_connected_locally():
                ...
                # [ LEFT, RIGHT, FRONT/BACK,  BALLAST, CALIBRATE ]
                ...
                # [ NAV_WP, BALLAST]
                elif len(data) == AUTONOMOUS_DATA_PACKET_LENGTH:
                    self.delta_times.append(time.time() - last_packet)
                    last_packet = time.time()
    ...
    def print_imu_data(self):
        ...
        print("Heading: %f, Roll: %f, Pitch %f" %
              (self.heading, self.roll, self.pitch), end="  ")
        print("X Accel: %f, Y Accel: %f, Z Accel: %f" %
              (x_accel, y_accel, z_accel), end="  ")
    ...
    def handle_autonomous_data(self, data):
        ...
        if data[START_BALLAST_INDEX]:
            # Number 1 for the data object in start_balance_sequence
            self.start_ballast_sequence(1)
        else:
        ...
        try:
            ...
            while self.is_radio_connected_locally():
                ...
                if len(current_data) == MANUAL_DATA_PACKET_LENGTH:
                    ...
                    return
                # [ NAV_WP, BALLAST]
                elif len(current_data) == AUTONOMOUS_DATA_PACKET_LENGTH:
                    # IF we recieved a different instruction set...
                    if str(original_data) != str(current_data):
    ...
    # Heading is the angle from North on a compass.
    def rotate_to_heading(self, heading):
        pass

    # X, Y is the long/lat we are travelling to; we know where we are; now get angle from 0
    def get_angle_from_north(self, x, y):
        pass
